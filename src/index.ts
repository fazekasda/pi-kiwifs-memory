...
// remain unchanged. Command registration and status text stay in index for
// the next workflow chunk.
export {
  buildSessionRuntime,
  crossOptInToScopes,
  openConfiguredBackend,
  resolveStateDir,
  setTokenizerNoteSink,
  type SessionRuntime,
...
// Q06B1: surface the runtime's tokenizer attach note through the existing
// status probe. Q06B2: the note is owned by the runtime instance itself
// (src/runtime/session.ts mutates `rt.tokenizerNote` after the async load),
// so the status probe reads the CURRENT runtime — a superseded session's
// late note can no longer leak through a module-global override. The
// `setTokenizerNoteSink` hook remains exported for API compatibility.
