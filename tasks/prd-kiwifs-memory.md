...
- [x] TUI checks pass; RPC/headless commands do not crash or emit terminal-only UI. _T18: synthetic ctx-level confirm true/false paths for every command; headless no-UI access verified per command; record-mutating headless commands require explicit `--yes`. Limit: interactive TUI confirm-dialog rendering not exercised live (no display in the environment) — reported, not fabricated. CLOSED in T19 chunk 3: live confirm-dialog gate exercised in an isolated real-terminal PTY session (herdr sidecar pane, automated harness, synthetic config against an unreachable loopback backend, zero model calls): `/kiwifs-forget` dialog rendered verbatim (title/path/reversibility note, Yes/No selector, key hints), explicit cancel → `forget cancelled`, explicit confirm → gated action executed and failed safely with a sanitized `transport fault` error. Evidence: `tasks/evidence/t19-tui-pty-dialog.txt` (labeled automated, not human signoff)._
...
- [x] Tests cover outage, restart, duplicate delivery, conflicting writes, malformed search/model output, superseded-recall leak (B3 incl. the read-back predicate with stale/empty tombstone cache), scope under-recall (B4) and brief-leg scope leakage, dropped vector jobs, and backend upgrades. _T19 chunk 1: integrated offline matrix `test/t19-fault-matrix.test.ts` runs the REAL pipeline (KiwiFSAdapter → fake MCP server, OutboxWorker, observation/backup/board senders, retrieval coordinator): outage→retry→recovery, crash-reload, replay no-op, conflicting write fail-closed (content-free quarantine), malformed payload quarantine, private-mode hold/release across all three features, adversarial retrieval (superseded read-back rejection incl. hybrid-surfaced records; out-of-scope exclusion; keyword-only hybrid degradation), bounded local state. T19 chunk 2 closes the remaining named gaps: brief-leg scope leakage as a DEDICATED case (out-of-scope + superseded + fabricated-body brief sections all fail-closed; brief text never trusted over the read-back; `project/demo-proj/…` paths made guard-real — the chunk-1 adversarial fixture's exclusions had been passing vacuously through guard step 4 and now assert the real predicates); malformed SEARCH output (mixed garbage lines fabricate no hits, phantom hits guard-rejected at read-back, canary-bearing noise never parses or leaks); dropped vector jobs (dropped INDEX job → record still recalled via FTS with the B4 under-recall measured 1-vs-2 and keyword-only hybrid degradation disclosed; dropped DELETE job → stale-vector ghost surfaced by the semantic leg is read-back-rejected); backend upgrades (additive tools/version tolerated capability-driven; required-tool removal fails closed BEFORE any mutation; additive output-shape evolution parses unchanged and a delivery cycle completes over the evolved shape). Also fixed in chunk 2: the observed live `kiwi_changes` IsError-500 defect silently killed board inbound delivery (feed-only discovery); delivery now falls back to ONE bounded MCP-only `kiwi_query_meta` listing pass per cycle on non-retryable feed domain rejections (availability faults still pause; cursor untouched; fallback + truncation disclosed in result/status/inbox; the outbox is outbound-only and pinned by test to never discover inbound messages) — architecture.md §8/§12, test/board-delivery.test.ts +8 tests, test/t19-fault-matrix.test.ts +4 tests, repo total 456 (up from 445)._
- [x] The opt-in live suite runs against the dedicated test space (MCP 8182) with random run IDs, manifest-owned cleanup verified even after partial failure, and bounded duration. _T19 chunk 3: executed via `KIWIFS_LIVE_TESTS=1 npm run test:live` (config consumed programmatically; no values/headers/URL printed). Outcome `clean-pass-with-degradations` (exit 5): 71 tools advertised, no required tool missing; routing check verified before mutation; synthetic create/read-back/update verified live with the ETag carrier present on both writes; FTS hit disclosed as async-indexing-pending; hybrid surfaced the record with `degraded=true, keyword only` (async vector indexing lag — disclosed, never semantic evidence); `kiwi_changes` is NON-FUNCTIONAL on this deployment (persistent server-side `internal server error (HTTP 500)` IsError whenever the feed has entries; empty feed without `last_seq` otherwise, while read-back proves the record exists) — disclosed as a degradation, never hidden; MCP-level delete + absence verified; manifest-owned cleanup with ZERO leftovers across every live run and probe. Runner defect fixed: cleanup previously shared the run's abort signal, so run-deadline expiry would orphan manifest-owned records — cleanup now uses a fresh bounded signal (pinned by test)._
- [x] Scope/privacy adversarial suite reports zero unauthorized content disclosures in fixtures. _T19 chunk 1: canary leak scanner proves every refusal/error/degradation string (worker quarantine fingerprints, enqueue gate, private-mode refusal, settle drop, adversarial pack) carries no synthetic secret or user-content canary._
- [x] Private-mode transition cancels/prevents pending sends according to policy across all features. _T19 chunk 1: observation + backup + board jobs all HELD with zero backend mutation while private; release on resume delivers all three (real adapter, fake server)._
- [x] Measured retrieval latency/context usage and extraction call volume meet T02 budgets. _T19 synthetic measurement harness `test/t19-budgets-quality.test.ts` over the REAL pipeline (KiwiFSAdapter → fake MCP server) and the scripted extract stub: latency — 5 retrieval cycles with 50 ms simulated backend latency per request measured 959–965 ms inside the 2 s total deadline; a 600 ms backend vs 400 ms deadline degrades at 401 ms with zero injection (deadline cuts the cycle, never overruns). Context — the cap-enforcement path runs with the synthetic tokenizer fixture (`test/fixtures/t19-synthetic-tokenizer.mjs`, word/punct, explicitly NOT production-model-compatible) counting the COMPLETE framed payload: 8 offered records → 6 kept, 2,645/3,000 tokens, 25-token framing baseline, deterministic re-count. Extraction volume — batch payload records the confirmed 6,000/3,000 budgets; a 60-entry backlog splits into 2 disjoint model calls (33+27) with complete union coverage; 6 repeated settles over unchanged coverage cost 1 call total. Measured values regenerate `tasks/evidence/t19-budget-report.json`; limits labeled: synthetic environment, no production latency/tokenizer/quality certification._
- [x] Retrieval fixture expectations and observation coverage criteria are documented and met; no invented quality percentages. _T19: `docs/t19-quality-baselines.md` documents the named deterministic fixtures and criteria, all asserted exactly in `test/t19-budgets-quality.test.ts` — retrieval QF-R1 exact path set in FTS score order, QF-R2 out-of-scope candidate guard-rejected at the `scope` step, QF-R3 superseded guard-rejected at the `status` step, QF-R4 keyword-only hybrid disclosed degraded and never counted as semantic, QF-R5 empty result → empty pack + visible note; coverage QF-C1 full-batch coverage (outbox `sourceEntryIds` == batch set), QF-C2 budget overflow splits into disjoint batches with complete union, QF-C3 queue-cap overflow merges into the oldest batch with `accepted ∪ pending == all entries` disjoint, QF-C4 repeated settles cost zero model calls, QF-C5 failed extraction retains the SAME opId and full durable coverage. Expectations are exact sets/counts and guard steps — no percentages; scripted-stub/synthetic-tokenizer limits disclosed; complement (not replacement) of the chunk-1/2 fault-matrix fixtures._
- [x] Long-session test demonstrates bounded local state and no active handles after shutdown. _T19 chunk 3 (final): `test/t19-long-session.test.ts` runs the REAL pipeline (fake MCP server + KiwiFSAdapter + durable outbox/worker + SessionCoordinator + ObserverScheduler with a scripted extract stub + BackupCapture + BoardDeliveryRuntime with its consumer lock + RetrievalCoordinator) through 60 cycles (120 extracted turns, one backup capture and one guarded retrieval per cycle) with private-mode windows, switch/tree transitions and full reload epochs; teardown mirrors the extension's `session_shutdown` order. Measured numerical evidence (regenerates `tasks/evidence/t19-long-session-report.json`): outbox journal 16,285 B after 60 cycles (retention prunes acked; 0 pending/quarantined; growth is manifest-payload proportionality, not leaks), observer state 40 B with 0 pending batches (20-batch cap), delivery dedupe set 2 entries (2,000 cap), coordinator consumed-coverage registry 120 entries / 829 B and backup state 10,558 B — both PROPORTIONAL to consumed entries / captured content by design (coverage correctness, architecture.md §3.3) and reported, not hidden; board delivery exactly-once across private windows and reloads. Handle audit: after session_shutdown the coordinator interval is stopped (`timerRunning=false`), the consumer lock is released and re-acquirable, the closed store refuses work, and `process.getActiveResourcesInfo()` returns to the pre-test baseline (zero Timeout, zero TCP sockets); a dedicated child process (`test/fixtures/t19-long-session-child.ts`) runs the full pipeline under REAL timers (10 ms tick interval + 10 ms board poll chain) through the same cycles and must EXIT BY DRAINING after shutdown — it exits in ~1.7–3 s with only its stdio pipe left (zero Timeout/TCPSocketWrap resources; numerical counts in the report). Two PRODUCTION defects found and fixed, not limit-bumped: (1) the observation scheduler enqueued extraction jobs with a fresh job opId while the payload carried the batch opId — the sender's `payload.opId === job.opId` validation quarantined EVERY scheduler-produced observation at delivery (the fault-matrix fixtures never delivered scheduler-produced jobs, so this was invisible); the job now carries the batch's durable opId; (2) the payload stored the ExtractionResult wrapper (`{observations:[...]}`) where the sender requires the observation ARRAY — every observation would also have failed validation; now normalized. PrivateModeGate bounded-state fixes: held-job refs are deduped by opId (the worker re-holds the same pending job every tick — the held set grew linearly with tick count) and the transition event log is FIFO-capped (100, metadata only); the fault-matrix private-mode fixture was corrected accordingly (its "held 3" had counted duplicate refs of ONE job — all three feature jobs now use distinct scopes so each is genuinely held). Limits disclosed: scripted extract stub and synthetic tokenizer; this measures state bounding and handle cleanup, NOT observation quality._
