...
- [ ] User approves architecture, the §13 default table, and the review resolutions in `docs/architecture-review.md`, or requested revisions are recorded before dependent implementation.
...
- [ ] Contract fixtures cover CRUD, search (FTS/semantic/hybrid incl. `keyword only` degradation), pagination, limits (limit clamp 50, 32 MiB content, 500-char path), `if_not_etag` reads, `kiwi_changes` cursor replay, the full B3 guard pipeline (read-back, status, scope, path, redaction — each step proven to reject its failure case, incl. stale/empty tombstone cache), op-id replay/collision fail-closed behavior, and the brief scope-gate fixture (`mcp-contracts.md` §9, 5a–5c).
- [ ] Timeouts and cancellation terminate pending requests within configured bounds (request-context cancellation propagates into backend calls).
- [ ] Authorization errors are not retried as transient failures; auth failures on the chosen endpoint are a hard setup error.
- [ ] Invalid/oversized backend responses produce typed safe failures.
- [ ] No constructor/factory performs network I/O.
- [ ] The opt-in live runner executes capability discovery, a synthetic CRUD/FTS round trip and manifest-owned cleanup against the dedicated test space (MCP 8182), never touching production spaces and never claiming VPN-only enforcement.
