...
## T20 chunk 1 — documentation reconciliation (2026, post-ff873a8)

Scope: reconcile user-facing docs with the implemented runtime; no behavior
changes except two cosmetic accuracy fixes (below). No commit, no
publication, no live/deployment action.

Files changed:

- `README.md` — rewritten: actual features, requirements (Pi 0.85.0, Node
  > =22.19.0 tested on 22.19.0+24, KiwiFS v0.19.62), tested minimal opt-in
  > config example, setup, commands/tools inventory, state-on-disk, layout,
  > explicit "Known limitations" (redaction heuristics, user-supplied
  > tokenizer requirement with no silent fallback, B4 under-recall,
  > network-protected MCP transport, reversible-only erasure, redacted
  > non-byte-identical backups with restore-into-Pi deferred, board labels
  > are not confidentiality, untested coexistence, not yet published).
- `docs/configuration.md` — NEW: full config reference cross-checked field
  by field against `src/config/schema.ts` (schema v1); credential
  by-reference rules; tokenizer module contract
  (`{id, countTokens(text) -> number | undefined}`, undefined = fail
  closed, deliberate NO bundled fallback for the default model route);
  scopes/identity/exclusions; board bounds (pollMs/backoffMs 5,000–
  3,600,000; backlogPauseAt 1–10,000; out-of-range = validation error, no
  silent clamp).
- `docs/operations.md` — NEW: outage/queue recovery (5,000 jobs / 50 MiB
  pause with visible coverage gap, 14-day retention acked-only, quarantine
  semantics, replay no-op vs collision fail-closed), private mode, backup
  verify/export rules and restore-into-Pi deferral, board delivery/GC
  (manual, local-only, acked/skipped >14d), forget/erasure limits (B6
  disclosure; true purge = operator procedure), backend-upgrade behavior,
  coexistence caveat, troubleshooting by status note, live-suite caveats
  (exit 5 = pass with disclosed degradations; dedicated test space is NOT
  a secure tenant boundary; auth header ≠ enforcement).
- `scripts/smoke-package.mjs` — cosmetic: final success message said
  "reports scaffold status"; now states what is actually asserted
  (registration + safe offline startup in isolated RPC, empty temp HOME,
  no backend). Assertions unchanged.
- `docs/publishing.md` — stale "current version is only a scaffold" replaced
  with the implemented-state pointer; publication still NOT performed.
- `package.json` — description no longer says "Development scaffold"
  (metadata only).
- `tasks/prd-kiwifs-memory.md` — T20 ACs marked with per-AC evidence,
  including the honest scope note that a live fresh-environment backend
  connection was not executed (no endpoint/credentials used).
- `tasks/evidence/t19-budget-report.json` — regenerated by the test run
  (latency jitter only, same values within budget).

Verification: config example validated against the REAL schema
(`validateConfig` → ok, defaults carried); secret scan of all changed/new
files clean (placeholder references only, no values); `npm run check` PASS
(472 tests), `npm run pack:check` PASS, `devenv test` PASS, exact Node
22.19.0 suite 472/472 PASS — all after final edits. No push, no tags, no
npm publish.

Not done in this chunk (remaining T20/ship work): independent review of
this chunk, final worker commit after review, and the user-approval gate
before any publication.

## T20 chunk 2 — package/release-candidate verification (this session)

Verification-only chunk on top of chunk 1's uncommitted changes. No commits made (final worker commits after independent review).

Findings and bounded fixes:

- **Runtime dependency gap (fixed).** `src/inject/tools.ts` and `src/board/tools.ts` import `typebox` at runtime (`Type.Object(...)` for tool schemas), but `typebox` was only present transitively via `@earendil-works/pi-coding-agent`'s optional peer tree. An isolated `npm install` of the packed tarball in a clean temp dir installed it only by hoisting luck. Fix: declared `"dependencies": { "typebox": "1.3.7" }` (exact, matching the resolved lockfile version) in `package.json` and regenerated `package-lock.json`. Re-verified: isolated install now pulls exactly 2 packages (extension + typebox), src/ shipped, optional peer not installed or forced.
- **Stale test name (fixed).** `test/extension.test.ts` still described the status test as "reports scaffold status...". Renamed to "status reports the disabled state without claiming memory works"; assertions unchanged.
- **`config/kiwifs-test.example.json` is NOT an extension config.** It validates as INVALID against `validateConfig` by design: it is the template for the T19 live runner (`KIWIFS_TEST_CONFIG`, old field shape `mcp.headers`/`space`/`safety`/`timeouts`). Reverted an initial wrong rewrite of it and instead documented this distinction in `docs/test-environment.md`. The README's minimal config example DOES validate against the real `validateConfig` (verified programmatically: VALID).
- Verified `npm package scopes/correctness`: `@fazekasda/pi-kiwifs-memory` scoped name, `publishConfig.access: "public"`, `private` absent, `pi-package` keyword, MIT LICENSE shipped, optional peerDependenciesMeta, engines `>=22.19.0` — all asserted by `scripts/check-package.mjs` which passed.
- Verified allowlist: tarball contains only `src/**/*.ts` + `package.json`/`README.md`/`LICENSE`; no config/, test/, docs/, tasks/, no local data or credentials. Runtime TS imports all included; test/docs tooling excluded as intended.
- Isolated load re-verified: packed tarball loads in Pi RPC from a clean temp HOME with no provider credentials; command registration + safe offline disabled-state startup asserted.

Gates (all re-run after the final edits of this chunk):

- `npm run check` — pass (typecheck + format + 472/472 tests).
- `npm run pack:check` — pass (allowlist + isolated RPC smoke with real registration and offline startup).
- `devenv test` — pass ("Tests passed :)").
- Exact Node floor `npx -y node@22.19.0 --test test/*.test.ts` — 472/472, fail 0.
- Isolated `npm install` of the packed tarball in a clean temp dir — pass (2 packages, typebox resolved, src present).
- `config/kiwifs-test.example.json` README-example schema validation — VALID.
- Secret scan of all files changed in chunk 1+2 (README, new docs, scripts, package files, test, tasks): clean. Only env-var-name references (`KIWIFS_MCP_APIKEY`, `OPENROUTER_API_KEY`) and placeholder values; private endpoint and its token appear in no tracked or newly written file.

Remaining: independent review, final worker commit, user-approval gate before publication. No push, tag, or npm publication performed.

## T20 final — post-review fixes and worker commit (this session)

Independent review of the uncommitted T20 chunks 1+2 (on ff873a8) returned
PASS with no blockers; all gates were re-run by the reviewer and are re-run
by the worker below rather than trusted from reports.

Bounded fixes after review:

- `docs/publishing.md` — dangling semicolon after "No publication has been
  performed as of T20" corrected to a period. Prose only.
- `tasks/evidence/t19-budget-report.json` — the suite regenerates this file
  on every run with the running Node version (`REPORT.nodeVersion` in
  `test/t19-budgets-quality.test.ts`), so an intermediate Node 24 run had
  overwritten the exact-floor pin that ff873a8 established. Fixed by gate
  ORDER, not by hand-editing: the exact Node 22.19.0 floor suite is the
  LAST full-suite run before staging, which regenerates the report with
  `nodeVersion` `v22.19.0` again. Remaining diff vs ff873a8 is latency
  jitter only, all values inside the measured budgets.
- `tasks/execution-log.md` — this entry.

Gate order after the final edit (measured this session, not trusted):

1. `npm run check` on local Node v24.19.0 — PASS (typecheck + format +
   472/472 tests). Node 24 compatibility measured here.
2. `npm run pack:check` — PASS (allowlist asserts only `src/**/*.ts` +
   manifest/README/LICENSE; isolated RPC smoke asserts real command
   registration and safe offline disabled-state startup from a clean temp
   HOME with no backend and no provider credentials).
3. `devenv test` — PASS (`npm ci` from the committed lockfile, then
   typecheck + format + tests + nixfmt + shellcheck + actionlint).
4. Isolated `npm install` of the packed tarball in a clean temp dir —
   PASS, exactly 2 packages (extension + `typebox@1.3.7`), re-verified by
   the worker because the reviewer had verified the lockfile resolution
   but not repeated the install.
5. Exact floor, LAST full-suite run: `npx -y node@22.19.0 --test
test/*.test.ts` — 472/472 PASS, fail 0; regenerates the budget report
   pinned to `v22.19.0`.

Honest remaining limitations (unchanged by this commit, all disclosed in
README/docs/PRD): a live fresh-environment backend connection was NOT
executed (policy: no endpoint/credential contact); live-suite exit 5 is a
pass-with-disclosed-degradations result; the dedicated test space is not a
secure tenant boundary; synthetic tokenizer/quality evidence is not a
production-model evaluation; redaction is best effort; erasure is
reversible forgetting only with no automatic erasure or board GC; backups
are redacted and not byte-identical with restore-into-Pi deferred; nothing
is published (no npm release, no tag, no push).

Secret scan of all staged files (changed + new): clean. Only env-var NAMES
