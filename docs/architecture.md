...
- Delivery-target pin (Q07): jobs enqueue with a target fingerprint (endpoint URL + credential-ref identity + record scope, sha-256 — never the secret value). A session-boundary rebuild against a CHANGED target holds retained jobs visibly instead of rerouting them; same-ref credential rotation delivers; legacy fingerprint-less jobs deliver as before (additive default). Worker checks the pin AFTER the private-mode gate, so privacy keeps precedence.
...
- **Changes-feed failure fallback (T19, disclosed degradation):** the feed is the primary inbound-discovery mechanism, but the live deployment has been observed failing `kiwi_changes` with a persistent server-side IsError HTTP 500 whenever the feed has entries (`tasks/evidence/t19-live-report.json`) — which would leave delivery DEAD (the outbox is outbound-only and never discovers inbound messages; this is pinned by test). When a poll cycle's feed call fails with a NON-retryable domain rejection (availability faults still pause), the cycle falls back to ONE bounded MCP-only `kiwi_query_meta` listing pass (filters `$.type=board-message`, limit/offset paging, ≤20 pages/≤1000 kept paths per cycle, strict board-path shape post-filter, offset-ignoring-backend guard). Every candidate still goes through the same fresh-read parse/TTL/dedupe/recipient pipeline, so the fallback discovers exactly the deliverable message set and no more. The stored cursor is untouched in fallback mode (dedupe absorbs overlap when the feed recovers); the fallback mode and any listing truncation are DISCLOSED in the cycle result, status snapshot and the `kiwifs_board_inbox` output — never a health claim.
...
| `kiwi_changes` feed domain failure (observed live: IsError 500 when populated) | board inbound discovery falls back to ONE bounded `kiwi_query_meta` listing pass per cycle, disclosed; availability faults still pause; cursor untouched; outbox is outbound-only and never a substitute |
